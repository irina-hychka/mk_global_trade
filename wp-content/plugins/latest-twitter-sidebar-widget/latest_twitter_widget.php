	...
	function widget($args, $instance) {
		...
		if( $haveTwitterData ){
			...
			foreach( $tweets as $tweet ){
                ...
                echo '<div class="info">';
                echo "<span class=\"author\">by " . $tweet->user->name . "</span>";

                if( $showTweetTimeTF ){
                    echo "<span class=\"tweet-time\" id=\"latest-twitter-tweet-time-" . $i . "\">" .  date('d.m.Y H:i', strtotime($tweet->created_at))  . "</span>";
                }
                echo '</div>';
				echo "<p class=\"tweet\">" . $this->fix_twitter_update( $tweet->text, $tweet->entities ) . "</p>";
